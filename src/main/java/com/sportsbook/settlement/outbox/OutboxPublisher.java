...
public class OutboxPublisher {
  ...
  private boolean sendBlocking(OutboxEvent event) {
    try {
      ...
      // Anything else: log and let the next tick retry. Duplicate sends are absorbed by the
      // consumer's idempotency model (bet.status transition is idempotent on betId).
      log.warn(
