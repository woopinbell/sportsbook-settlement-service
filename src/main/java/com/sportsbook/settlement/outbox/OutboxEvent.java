...
/**
 * Row in the transactional outbox (ADR-0006). Written in the same DB transaction as the settlement
 * transition (bet -> SETTLED / VOIDED) so the event cannot diverge from the underlying state
 * change. {@link OutboxPublisher} polls for unpublished rows, sends them to Kafka, and stamps
 * {@link #publishedAt()} on success.
 *
